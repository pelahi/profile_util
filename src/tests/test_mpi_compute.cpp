...
void usage()
{
    ...
    Rank0Log()<<"  -n <number of points per dim> (default 256) "<<std::endl;
    Rank0Log()<<"  -i <number of iterations> (default 10) "<<std::endl;
    Rank0Log()<<"  -d <fraction of grid spacing particles are moved as positions update. Large deltap will move more particles across MPI domains> (default 0.1) "<<std::endl;
    Rank0Log()<<"  -c <compute flag> (default 1) "<<std::endl;
    Rank0Log()<<"  -v <verbose flag> (default 0) "<<std::endl;
    MPI_Finalize();
    exit(0);
}
...
{
    int option;
    while ((option = getopt(argc, argv, ":n:i:d:c:v:")) != EOF)
    {
        switch(option)
        {
            case 'n':
                opt.npoints = atoll(optarg);
                break;
            case 'i':
                opt.Niter = atoi(optarg);
                break;
            case 'd':
                opt.deltap = atof(optarg);
                break;
            case 'c':
                opt.icompute = atoi(optarg);
                break;
            case 'v':
...
int main(int argc, char **argv) {
    ...
    {
        Rank0Log()<<"At iteration "<<i<<std::endl;
