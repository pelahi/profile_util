...
__global__ void vec_silly(const int N, const int Ni, const float * __restrict__ din, float * __restrict__ dout){
    // Calculate the unique global thread index
    int index = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
    if (index >= N) return;
    for (int i=0;i<Ni;i++) {
        double delta = 0;
        if (i==0 || i==N-1) delta = din[index] ;
        else delta = din[index]-din[index+1];  
        dout[index] = din[index]+din[index]*index / pow(1.1,i) + sin(din[index])+din[index]*delta;
    }
}
...
void usage()
{
    ...
    Rank0Log()<<"  -l <number of loops for each test> (default 100) "<<std::endl;
    ...
    Rank0Log()<<"     kernel type 3: silly vec operation with inner loop"<<std::endl;
    Rank0Log()<<"  -d <comma separated list of gpu devices to use> (default -1 [all devices]) "<<std::endl;
    Rank0Log()<<"  -s <sleep time in seconds> (default 0.0) "<<std::endl;
    Rank0Log()<<"  -S <sampling time in seconds> (default 0.001) "<<std::endl;
#ifdef _MPI
    MPI_Finalize();
#endif
    exit(0);
...
{
    int option;
    while ((option = getopt(argc, argv, ":n:i:l:b:t:k:d:s:S:")) != EOF)
    {
        switch(option)
        {
            case 'n':
            ...
            case 's':
                opt.sleep_time = atof(optarg);
                break;
            case 'S':
                opt.sampling_time = atof(optarg);
                break;
            case '?':
                usage();
                break;
        }
        if (opt.blocksize == -1) opt.blocksize = (opt.Nentries + opt.threadsperblock - 1) / opt.threadsperblock;
    }
}
...
int main(int argc, char *argv[])
{
    ...
    Log()<<"Running "<<opt.Nentries<<" computations for "<<opt.Niter<<" iterations on gpu devices using kernel"<<opt.kernel_type<<" with "<<opt.Nloop<<" loops"<<std::endl;
    ...
    Log()<<s<<std::endl;
    Log()<<"Block size "<<opt.blocksize<<" with "<<opt.threadsperblock<<" threads per block"<<std::endl;

    Log()<<"Starting time and compute samplers"<<std::endl;
    auto t1 = NewTimer();
    auto s1 = NewComputeSampler(opt.sampling_time);
    size_t shsize = 0;
...
#pragma omp parallel for reduction(+:sum) schedule(static)
    ...
    if (opt.sleep_time>0) {
        Log()<<"Sleeping for "<<opt.sleep_time<<" seconds before starting GPU computation"<<std::endl;
    ...
    for (auto &idev:opt.device_list) 
    {
        ...
        auto sgpu = NewComputeSampler(opt.sampling_time);
        for (auto iter=0;iter<opt.Niter;iter++) {
            ...
            else if (opt.kernel_type == 2) {
                pu_gpuLaunchKernel(vec_add_silly,
                    dim3(opt.blocksize), dim3(opt.threadsperblock), 
                    shsize, stream,
                    opt.Nentries, opt.Nloop, xdev[idev], ydev[idev]);
            }
            else if (opt.kernel_type == 3) {
                pu_gpuLaunchKernel(vec_silly,
                    dim3(opt.blocksize), dim3(opt.threadsperblock), 
                    shsize, stream,
                    opt.Nentries, opt.Nloop, xdev[idev], ydev[idev]);
            }
