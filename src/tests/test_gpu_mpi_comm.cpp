...
void usage()
{
    ...
    Rank0Log()<<"  -G <1 to test gpu-gpu comm> "<<std::endl;
    Rank0Log()<<"  -C <1 to test cpu-cpu comm> "<<std::endl;
    MPI_Finalize();
    exit(0);
}
    ...
    while ((option = getopt(argc, argv, ":s:i:r:o:G:C:")) != EOF)
    {
        switch(option)
        {
            ...
            case 'o':
                ...
                break;
            case 'G':
                opt.igpu = atoi(optarg);
                break;
            case 'C':
                opt.icpu = atoi(optarg);
                break;
            case '?':
                usage();
                break;
...
void MPIFreeComms(std::vector<MPI_Comm> &mpi_comms, std::vector<std::string> &mpi_comms_name){
    for (auto i=0u;i<mpi_comms.size()-1;i++) {
...
std::vector<unsigned long long> MPISetSize(double maxgb) 
{
    ...
    for (auto i=1u;i<sizeofsends.size();i++) sizeofsends[i] = sizeofsends[i-1]/8;
...
void MPITestCPUSendRecv(Options &opt) 
{
    ...
    LogMPITest();
    for (auto i=0u;i<sizeofsends.size();i++) 
    {
        ...
        p2 = receivedata.data();
        auto time1 = NewTimer();
        for (auto j=0u;j<mpi_comms.size();j++)
        {
#ifdef TURNOFFMPI
#else
...
void MPITestGPUCorrectSendRecv(Options &opt){
    ...
    for (auto i=0u;i<sizeofsends.size();i++) 
    {
        ...
        for (auto idev=0; idev<nDevices;idev++) {
            pu_gpuErrorCheck(pu_gpuSetDevice(idev));
            p1 = gpu_p1[idev];
            Rank0Log()<<"Communicating using device "<<idev<<std::endl;
            if (ThisTask == opt.roottask) {
                for (auto itask = 0;itask<NProcs;itask++) {
                    if (itask == opt.roottask) continue;
                    int mpi_err=0;
                    ...
                    for (auto i=0ull;i<sizeofsends[i];i++) {
